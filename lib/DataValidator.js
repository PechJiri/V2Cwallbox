...
class DataValidator {
    ...
    _initializeSchema() {
        this.schemaCache.set('processedDataSchema', {
            ...
            evchargerChargingState: value => typeof value === 'string' && Object.values(CONSTANTS.EVCHARGER_STATES).includes(value),
            evchargerCharging: value => typeof value === 'boolean',
            onoff: value => typeof value === 'boolean',
            alarmGeneric: value => typeof value === 'boolean'
        });
    }
    ...
    processData(rawData) {
        ...
        // Odvozené capabilities pro standardní Homey EV Charger
        result.evchargerChargingState = this._deriveEvChargerState(result.chargeState, result.paused);
        result.evchargerCharging = result.evchargerChargingState === CONSTANTS.EVCHARGER_STATES.PLUGGED_IN_CHARGING;
        result.onoff = !result.paused;
        result.alarmGeneric = result.slaveError !== CONSTANTS.SLAVE_ERRORS.NO_ERROR;

        return result;
    }

    _deriveEvChargerState(chargeState, paused) {
        if (chargeState === CONSTANTS.CHARGE_STATES.DISCONNECTED) {
            return CONSTANTS.EVCHARGER_STATES.PLUGGED_OUT;
        }
        if (paused) {
            return CONSTANTS.EVCHARGER_STATES.PLUGGED_IN_PAUSED;
        }
        if (chargeState === CONSTANTS.CHARGE_STATES.CHARGING) {
            return CONSTANTS.EVCHARGER_STATES.PLUGGED_IN_CHARGING;
        }
        return CONSTANTS.EVCHARGER_STATES.PLUGGED_IN;
    }
