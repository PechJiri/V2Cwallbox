...
class DataValidator {
    ...
    getSchema() {
        if (!this.schemaCache.has('processedDataSchema')) {
            const schema = {
                chargeState: value => typeof value === 'string' && DataValidator.CHARGE_STATES.includes(value),
                ...
                minIntensity: value => typeof value === 'number' && value >= DataValidator.INTENSITY_LIMITS.MIN && value <= DataValidator.INTENSITY_LIMITS.MAX,
                maxIntensity: value => typeof value === 'number' && value >= DataValidator.INTENSITY_LIMITS.MIN && value <= DataValidator.INTENSITY_LIMITS.MAX,
                firmwareVersion: value => typeof value === 'string',
                signalStatus: value => typeof value === 'string' && DataValidator.SIGNAL_STATES.includes(value),
    ...
    processChargeState(state) {
        return state === undefined || state === 4 ? '0' : String(state);
    }
