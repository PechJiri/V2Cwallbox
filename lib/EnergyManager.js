...
class EnergyManager {
    ...
    async initialize() {
        ...
        // Best-effort seed pro existující uživatele: odhad lifetime z dostupných ročních dat.
        // Nová instalace začne od 0.
        if (!this.lifetimeData || typeof this.lifetimeData.energy !== 'number') {
            const yearlyEnergy = (this.yearlyData && typeof this.yearlyData.energy === 'number')
                ? this.yearlyData.energy : 0;
            const lastYearEnergy = await this.device.getStoreValue('lastYearEnergy') || 0;
            const seed = yearlyEnergy + lastYearEnergy;
            ...
            if (this.logger) {
                this.logger.debug('Inicializace lifetime čítače', {
                    seed,
                    fromYearly: yearlyEnergy,
                    fromLastYear: lastYearEnergy
                });
            }
        }
    }
    ...
    getLifetimeEnergy() {
        return (this.lifetimeData && typeof this.lifetimeData.energy === 'number')
    ...
    async handleEnergyReset(currentEnergy, lastKnownEnergy) {
        if (currentEnergy < lastKnownEnergy - 0.1) {
            ...
            // chargingStartEnergy musí jít na 0 spolu s base — jinak by processDisconnection
            // při odpojení sečetl celou novou session ještě jednou (dvojí započtení po API jitteru).
