...
module.exports = {
    ...
    // API konfigurace
    API: {
        TIMEOUT: 6000,                    // Timeout pro API požadavky v ms
    ...
    INTERVALS: {
        ...
        BACKOFF_MAX: 300000,  // Maximální interval při backoff (5 min)
        BACKOFF_MULTIPLIER: 3 // Násobitel pro exponential backoff (5s → 15s → 45s → ...)
    },
    ...
    // Hodnoty pro Homey SDK capability evcharger_charging_state
    EVCHARGER_STATES: {
        PLUGGED_OUT: 'plugged_out',
        PLUGGED_IN: 'plugged_in',
        PLUGGED_IN_PAUSED: 'plugged_in_paused',
    ...
    // Mapování mezi Homey systémovou capability target_power_mode a V2C parametry
    // (Dynamic flag + DynamicPowerMode). 'homey' znamená Homey přebírá kontrolu
    // (Dynamic=0), ostatní hodnoty jsou konkrétní V2C dynamic strategie (Dynamic=1).
    TARGET_POWER_MODES: {
        HOMEY: 'homey',
        V2C_TIMED_ON: 'v2c_timed_on',
        V2C_TIMED_OFF: 'v2c_timed_off',
        V2C_FV_EXCLUSIVE: 'v2c_fv_exclusive',
        V2C_FV_MIN: 'v2c_fv_min',
        V2C_GRID_FV: 'v2c_grid_fv',
        V2C_NO_CHARGE: 'v2c_no_charge'
    },

    // target_power_mode → V2C DynamicPowerMode ('0'-'5'). Klíč 'homey' zde nemá
    // protějšek (Dynamic se nastaví na '0' a DynamicPowerMode se nemění).
    TARGET_MODE_TO_V2C: {
        v2c_timed_on:     '0',
        v2c_timed_off:    '1',
        v2c_fv_exclusive: '2',
    ...
    // Opačný směr — V2C DynamicPowerMode ('0'-'5') → target_power_mode.
    // Používá se v updateCapabilities při čtení stavu z V2C (když Dynamic=1).
    V2C_TO_TARGET_MODE: {
        '0': 'v2c_timed_on',
        '1': 'v2c_timed_off',
        '2': 'v2c_fv_exclusive',
